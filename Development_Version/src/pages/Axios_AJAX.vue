    ...
    <div class="input-group">
      ...
      <div class="input-group-append">
        <button
          type="button"
          class="btn btn-info"
          @click="url = `${__proto__.mobileurl}axios-server?name=niki&age=22`"
        >
          切换URL
        </button>
...
export default {
  ...
  data() {
    return {
      ...
      countDown: 3,
      timer: null,
      url: "http://127.0.0.1:8000/axios-server?name=niki&age=22",
      isSuccess: false,
    };
  },
  computed: {
  ...
  methods: {
    send() {
      ...
      axios({
        url: this.url,
        method: "POST",
        data: {gender: "male", school: "MIT"}, //请求体
        params: {xxx: "xxx", yyy: "yyy"}, //参数
        ...
        .then((response) => {
          ...
          this.responseContent =
            response.data.msg + "\n" + response.data.title + JSON.stringify(response.data.data);
