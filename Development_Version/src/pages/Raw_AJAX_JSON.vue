    ...
    <div class="input-group">
      ...
      <div class="input-group-append">
        <button
          type="button"
          class="btn btn-info"
          @click="url = `${__proto__.mobileurl}json-server`"
        >
          切换URL
        </button>
...
export default {
  ...
  computed: {
    ...
    buttonInfo() {
      return this.buttonActive ? `请稍后重试(${this.countDown})` : "请求JSON数据";
    },
  },
  methods: {
    send() {
      ...
      //3. 设置响应体的数据类型（express会将后端的json数据自动转为string，设置responseType为json，在收到数据后又会转回json）
